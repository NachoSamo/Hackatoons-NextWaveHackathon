...
| 6 | A Mexican issuer fails for one merchant at the same time. Centinel creates a second incident without mixing causes | Changes the dimensional scope to `Rappido × Mexico` and simulates the second signal | ⬜ | Samo + Luca |
...
| 8 | Ambiguous or low-sample case: the system shows `Insufficient evidence`, alternatives and missing data | Uses the weak-evidence path once it is reconnected to the dimensional flow | 🟡 | Luca + Juani |
| 9 | Trial by fire: the judge selects and triggers a new dimensional combination; the system detects, localizes and explains without team intervention | Hands the same `Detection filters` panel to the judge | ✅ demo | Whole team |
...
- [x] The judge receives dimensional dropdowns and never raw JSON; option discovery from backend remains pending
